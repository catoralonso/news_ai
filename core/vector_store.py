...
class VectorStore:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(
                "ChromaDB PersistentClient falló al inicializar. "
                "persist_dir=%s | error_type=%s | detalle=%s",
                persist_dir, type(e).__name__, e,
                exc_info=True,
            )
            raise
        ...
        try:
            ...
            logger.error(
                "ChromaDB get_or_create_collection falló. "
                "collection=%s | error_type=%s | detalle=%s",
                collection_name, type(e).__name__, e,
                exc_info=True,
            )
            raise
