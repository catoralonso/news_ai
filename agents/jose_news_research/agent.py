...
from config import EMBEDDINGS_DIR, NEWSPAPER_NAME, REGION as PAIS
...
""".strip()
    ...
    def _call_llm(self) -> str:
            messages = [
                ...
                for m in self.memory.as_messages()
            ]
            return call_llm(
                prompt=messages,
    ...
    def _system_prompt(self) -> str:
        return self.SYSTEM_PROMPT.replace("{newspaper}", self.newspaper_name).replace("{region}", self.region)
