...
Architecture:
    ...
    ReaderInteractionAgent.chat(user_input)
          │
          ├─► _detect_intent()              ← lightweight LLM call (no RAG, no memory)
          │         │
          │    "fact_check" ──────────────► Camila.verify_url(input_text)
          │    "question"   ──────────────► KnowledgeBase.retrieve() + Claude
          │    "other"      ──────────────► Claude (friendly deflection)
          │
          └─► ReaderResponse
                  ├── message
                  ├── intent
...
from config import NEWSPAPER_NAME, REGION as PAIS, ESCALATION_LOG
...
class ReaderInteractionAgent:
    ...
    ┌────────────────────────────────────────────────────────────────────┐
    │ 1. Detect intent (lightweight Claude call, no memory)              │
    │ 2a. fact_check → verify_url() via injected Camila instance         │
    │ 2b. question   → RAG + LLM call, recommend article, maybe escalate │
    │ 2c. other      → friendly Claude response                          │
    ...
    SYSTEM_PROMPT = """
You are Mauro, an italian nutrition reader interaction assistant working for 
a local spanish nutrition newspaper.
...
""".strip()
    ...
    def _detect_intent(self, user_input: str) -> str:
        """
        Lightweight Claude call to classify reader input.
...
""".strip()
    ...
    def _handle_question(self, user_input: str, intent: str) -> ReaderResponse:
        ...
        Retrieves RAG context, calls Claude, recommends an article,
...
""".strip()
        ...
        # 4. Escalate if: no RAG context AND Calude gave a very short/uncertain reply
    ...
    def _call_llm(self) -> str:
        """Calls Claude with the current memory and returns the response text."""
        raw_text = call_llm(
    ...
    def _escalate(self, user_input: str) -> None:
        ...
        log_dir = os.path.dirname(escalation_log)
        if log_dir:
